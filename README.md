...
[![tests](https://img.shields.io/badge/tests-93_passing-0078D7?style=flat-square)](https://github.com/victorzhong0110/da-verify/actions/workflows/ci.yml)
[![python](https://img.shields.io/badge/python-%E2%89%A5%203.11-0078D7?style=flat-square)](pyproject.toml)
...
40 program-checkable data-analysis tasks (InfiAgent-DABench); same model, harness, and grader across conditions:

- **C3 — self-consistency + programmatic agreement gate:** pass@1 **84.5%**, **+20.0pt vs C0** (paired bootstrap 95% CI **[+10.5, +30.0]**) at temp 0.7 / k=5. No LLM ever judges anything.
- **C1 — same-model self-check:** **Δ 0.0%** (McNemar p = 1.00). A clean null.
- **C2 — independent LLM re-derivation:** **+11.0pt** at temp 0.7 / k=5; statistically invisible at temp 0 / k=1.
- **Conclusion:** verification repairs **variance, not capability**. The active ingredient is sample diversity plus programmatic reconciliation — not LLM judgment. Failures land where the theory says they must: non-reproducible gold, and confident-wrong systematic errors.

> 中文摘要：在可被程序校验的数据分析任务上，验证修复的是采样方差而非模型能力——自我检查完全无效（Δ 0.0%）；有效成分是采样多样性 + 程序化一致性判定，C3 比 LLM 验证器还高 +9.0pt。

<img src="assets/c0-c3.svg" alt="C0–C3 results: nothing moves at temp 0 / k=1; at temp 0.7 / k=5 the programmatic verifier C3 reaches 84.5% pass@1, +20.0pt over C0 with CI [+10.5, +30.0]" width="100%"/>

<sub>Figure regenerable via `python3 scripts/make_headline_figure.py` — numbers from `report/report.md`.</sub>
...
## Development log — how the result was earned

The study was built week by week (W1→W7); W7 above is the outcome. The earlier
milestones are kept here because the *process* — including two harness artifacts
caught by error analysis — is part of the evidence.

<details>
<summary><strong>W1 — the verifier (the foundation)</strong></summary>
...
<details>
<summary><strong>W2 — C0 baseline end-to-end</strong> · ⚠️ <em>superseded: the 10-task 60% below was small-sample noise; W3's full-set figure is 82.5%</em></summary>
...
<details>
<summary><strong>W3 — stable full-set eval + pass@k harness</strong></summary>
...
<details>
<summary><strong>W4 — C1 self-verification (a clean null at temp 0)</strong></summary>
...
<details>
<summary><strong>W5 — C2 external verification, and the C0/C1/C2 picture</strong></summary>
...
<details>
<summary><strong>W6 — multi-model verification + two artifacts honest analysis caught</strong></summary>
